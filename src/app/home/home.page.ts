...
export class HomePage {
      ...
      this.healthKit.available().then(available => {
        if (available) {
          ...
          var options: HealthKitOptions = {
            readTypes: ['HKQuantityTypeIdentifierHeartRate', 'HKQuantityTypeIdentifierHeight', 'HKQuantityTypeIdentifierStepCount', 'HKWorkoutTypeIdentifier', 'HKQuantityTypeIdentifierActiveEnergyBurned', 'HKQuantityTypeIdentifierDistanceCycling'],
            writeTypes: ['HKQuantityTypeIdentifierHeartRate', 'HKQuantityTypeIdentifierHeight', 'HKWorkoutTypeIdentifier', 'HKQuantityTypeIdentifierActiveEnergyBurned', 'HKQuantityTypeIdentifierDistanceCycling']
          }
  ...
  loadHealthData() {
    ...
    this.healthKit.querySampleType(heardRateOptions).then(data => {
      let stepSum = data.reduce((a, b) => a + b.quantity, 0);
      this.stepcount = stepSum;
    }, err => {
